...
const seed = async () => {
  ...
  await CrowdZone.insertMany(
    zoneNames.map((z) => ({
      stadium: metlife._id,
      ...z,
      currentCount: Math.floor(z.capacity * (0.3 + Math.random() * 0.5)),
      ...
      status: z.density >= 85 ? 'critical' : z.density >= 70 ? 'congested' : z.density >= 50 ? 'moderate' : 'normal',
    }))
  );
